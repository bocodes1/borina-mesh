# Wiki Engine — Phase C Implementation Plan (Big Bang Migration)

> **For agentic workers:** This runs as one big migration agent dispatch, not TDD tasks.

**Goal:** Read the entire existing Obsidian vault, extract all signal-worthy content into the new wiki structure (entities/concepts/decisions), bootstrap `index.md` + `log.md`, archive the originals, seed `curator-memory.md` with patterns observed from the existing content.

**Approach:** One large migration agent dispatch with a carefully-curated system prompt. The migration agent reads in passes, proposes changes, and writes them through the Borina `/memory/propose` endpoint (so the reviewer sees each one and can apply audit entries consistently).

**Prerequisite:** Phase A must be deployed on Mac Mini before running Phase C. The migration script talks to the live Borina API.
...
**Files:**
- Create: `borina-mesh/scripts/migrate_vault.py`

- [ ] **Step 1: Write migrate_vault.py**
...
git add scripts/migrate_vault.py
git commit -m "feat(wiki): big-bang migration script (scripts/migrate_vault.py)

Walks the existing Obsidian vault, chunks every .md/.txt/.org file, and
posts each chunk as a proposal to /memory/propose. Optional --archive
moves processed files into _archive/. Optional --review-after triggers
batch review runs until the queue drains.

Use: python scripts/migrate_vault.py --vault /Users/clawd/.openclaw/workspace --api http://localhost:8000 --archive --review-after
"
...
**Files:** none (just execution)

- [ ] **Step 1: Dry-run locally (PC first, limited files)**

From the PC, against the Mac Mini API:

```bash
cd /c/Users/wenbo/OneDrive/Desktop/CLAUDECODE/borina-mesh
python scripts/migrate_vault.py \
  --vault "/c/Users/wenbo/OneDrive/Documents/remote" \
  --api "http://100.116.121.128:8000" \
  --max-files 5 \
  --dry-run
```

Verify output shows 5 files found, chunks computed, no network activity.

- [ ] **Step 2: Small real run (5 files, no archive)**
...
Check `_queue/pending/` on Mac Mini (or via `GET /wiki/status`) — should show new items.

- [ ] **Step 3: Trigger review on the small batch**

```bash
curl -X POST "http://100.116.121.128:8000/wiki/review?max_items=20"
```

Check `_queue/approved.jsonl` and the new pages under `entities/`, `concepts/`, `decisions/`.

- [ ] **Step 4: Spot-check output quality**

Open a few generated pages in Obsidian. Verify they have sensible frontmatter, real content, cross-references. If the reviewer is filtering too aggressively or missing signal, adjust `curator-memory.md` manually and re-run a small batch to see if behavior changes.

---

## Task 3: Full migration

**Files:** none (execution only)

- [ ] **Step 1: Full run with archive + review**
...
This will take 20-60 min depending on vault size. Watch the Borina dashboard (/terminal and /jobs) for live progress.

- [ ] **Step 2: Verify index.md and log.md got populated**

Open `index.md` in Obsidian. Should have dozens of entries organized by page type. `log.md` should have one entry per approved batch.

- [ ] **Step 3: Review curator-memory.md**

The reviewer should have appended ~10-30 learned patterns during the migration. Read them and decide if any need manual correction or promotion.

- [ ] **Step 4: Check _archive/**

All original files should now live under `_archive/` with their original structure preserved.

---

## Post-Migration

After the migration completes, the reviewer continues to run on schedule (every 30 min) and whenever a new proposal is queued. New Borina agent runs and Claude Code sessions will all flow through the same curation pipeline.

### Rollback

If you need to rollback the migration:
```bash
# Copy originals back out of _archive
cp -r $VAULT/_archive/* $VAULT/
# Delete the new wiki structure
rm -rf $VAULT/entities $VAULT/concepts $VAULT/decisions $VAULT/sources $VAULT/_queue
rm $VAULT/index.md $VAULT/log.md $VAULT/curator-memory.md $VAULT/00-schema.md
```
